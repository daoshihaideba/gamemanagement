...
pro.startGame = function () {
    ...
    async.series([
        function (cb) {
            rpcAPI.getCurRobotWinRate(self.gameTypeInfo.kind, self.gameTypeInfo.level, function (err, rate) {
                if (!!err) {
...
pro.recordGameResult = function (dataArr, cb) {
    ...
    for (let i = 0; i < dataArr.length; ++i) {
        ...
        // 记录广播内容 捕鱼在桌子里面记录
        if (tempScore >= 1000 && this.gameTypeInfo.kind !=enumeration.gameType.FISH) {
   ...
   this.broadcastContentArrPush(null,null,broadcastContentArr);
    // 写入分数
    let self = this;
    ...
    // 记录库存值变化 //捕鱼库存在这里计算
    if (systemGoldChange !== 0 && this.gameTypeInfo.kind != enumeration.gameType.FISH) {
        rpcAPI.robotGoldChanged(this.gameTypeInfo.kind, systemGoldChange, this.gameTypeInfo.level, function (err) {
            if (!!err) {
...
pro.broadcastContentArrPush = function(Score,user,broadcastContentArr){
     // 记录广播内容 捕鱼在桌子里面记录
     if(!broadcastContentArr && Score && user){
         broadcastContentArr = [];
        if (Score >= 1000 ) {
           broadcastContentArr.push({
               nickname: user.userInfo.nickname,
     ...
     // 推送广播
     if (broadcastContentArr && broadcastContentArr.length > 0) {
...
pro.robotGoldChanged = function (systemGoldChange) {
    rpcAPI.robotGoldChanged(this.gameTypeInfo.kind, systemGoldChange, this.gameTypeInfo.level, function (err) {
        if (!!err) {
            logger.error("recordGameResult", 'robotGoldChanged err:' + err);
        }
...
pro.robotOperationRoomCreated = function () {
    ...
    if (!!this.gameTypeInfo.matchRoom) {
        ...
        this.requestRobotTimer = setTimeout(function () {
            ...
            console.log("匹配房间机器人开始工作", needMinRobotCount, needMaxRobotCount)
