...
logic.isFirstOut = function(cardDataArr){
    for (let i = 0; i < cardDataArr.length; ++i){
        if (cardDataArr[i] === 0x03) return true;
    }
    return false;
};
...
logic.getCardType = function (cardDataArr) {
    ...
    if (analyseResult.fourCardData.length > 0) {
        ...
        if (analyseResult.fourCardData.length === 4 && cardCount >= 5 && cardCount <= 8){
    ...
    if (analyseResult.threeCardData.length > 0) {
        ...
        if (threeCount * 5 === cardCount) return gameProto.cardType.THREE_LINE_TAKE_TWO;

        return gameProto.cardType.ERROR;
    }
    //两张类型
    if (analyseResult.doubleCardData.length/2 >= 2) {
...
logic.compareCard = function (firstCardArr, nextCardArr) {
    ...
    switch (nextType) {
        ...
        case gameProto.cardType.FOUR_LINE_TAKE_X:{
            //分析扑克
            let nextResult = this.analyseCardDataArr(nextCardArr);
            let firstResult = this.analyseCardDataArr(firstCardArr);
            //获取数值
...
logic.searchOutCard = function(handCardDataArr, turnCardDataArr) {
    ...
    switch (turnOutCardType) {
        ...
        case gameProto.cardType.THREE_LINE_TAKE_ONE:	//三带1
        case gameProto.cardType.THREE_LINE_TAKE_TWO:	//三带2
        {
            //长度判断
            if (cardCount<turnCardCount) break;
            ...
            for (let i=turnLineCount*3-1;i<cardCount;i++){
                ...
                for (let j=(cardCount-i-1);j<(cardCount-2);j++){
                    ...
                    if (lineCount === turnLineCount) {
                        ...
                        // 移除已经在三连中的牌，防止构成炸弹
                        let removeCardArr = [];
                        for (let k = 0; k < leftCardDataArr.length; ++k){
                            let logicValue = this.getCardLogicValue(leftCardDataArr[k]);
                            let isExist = false;
                            for (let h = 0; h < resultCardArr.length; ++h){
                                if (logicValue === this.getCardLogicValue(resultCardArr[h])){
                                    isExist = true;
                                    break;
                                }
                            }
                            if (isExist){
                                removeCardArr.push(leftCardDataArr[k]);
                            }
                        }
                        if (removeCardArr.length > 0){
                            this.removeCard(removeCardArr, leftCardDataArr);
                        }
